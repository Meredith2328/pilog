...
DINO_EMBED_SCRIPT = (
    ...
    "s.textContent='body{align-items:flex-start;justify-content:center;background:#f7f7f7}."
    "page{padding:10px 8px 12px;gap:10px}.hint,.credit{display:none!important}.stage{overflow:hidden}';"
    "document.head.appendChild(s);"
    "var tt=document.createElement('style');tt.textContent='.toolbar{display:none!important}';"
    "function ts(){var n=window.top&&window.top.innerWidth<860;"
    "if(n&&tt.parentNode!==document.head)document.head.appendChild(tt);"
    "if(!n&&tt.parentNode===document.head)document.head.removeChild(tt);}"
    "ts();window.addEventListener('resize',ts);"
