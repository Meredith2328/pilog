...
<div class="algoviz" data-module="lc1-两数之和" data-title="1 两数之和 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc49-字母异位词分组" data-title="49 字母异位词分组 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc128-最长连续序列" data-title="128 最长连续序列 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc128-最长连续序列-v2" data-title="128 最长连续序列 · 步骤可视化"></div>
```python
class Solution:
...
<div class="algoviz" data-module="lc283-移动零" data-title="283 移动零 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc11-盛最多水的容器" data-title="11 盛最多水的容器 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc15-三数之和" data-title="15 三数之和 · 步骤可视化"></div>
```
class Solution:
...
十分经典的滑动窗口入门题。滑动窗口总是用**右指针用来遍历**新元素、直到不满足某个条件，然后**收缩左指针**直到恰好又满足条件。

时间复杂度：$O(n)$ ，空间复杂度：$O(\min \{n, |\Sigma|\})$ 。这是因为 `visited` 哈希集合中出现的元素数量一定不会超出字符集的数量（如26），同时也不会超出 `s` 的长度。
...
class Solution:
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        left = 0
        maxLen = 0
        for right, ch in enumerate(s): # 闭区间窗口 [left, right]
            while ch in visited:
...
数组访问是比哈希要快的，两个大小为26的数组相等判断我们也默认它是 $O(1)$ 的。所以有下述写法。

> **固定大小的滑动窗口**的写法：
>
> 开头添加元素进去。
>
> 到达要稳定下来的大小之后，进一个、退一个。

另外还有个python语法，没有什么神秘的，就是字符和ASCII编号的互转：字符转数字用 `ord` ，数字转字符用 `chr` 。

时间复杂度：$O(n)$ ，空间复杂度：$O(1)$ 。

或者更精细地也可以理解成时间复杂度：$O(m+(n-m)|\Sigma|)$ ，空间复杂度：$O(|\Sigma|)$ ，时间复杂度是因为初始化为 $O(m)$ ，而每次判断为 $O(|\Sigma|)$、需要 $n-m$ 次判断。其中 $n$ 和 $m$ 分别是 `s` 和 `p` 的长度，词汇表$|\Sigma|=26$ 。

当然，这种方法还可以略微优化：把统计频率改成统计 `diff` 。这里就不写了。

```python
class Solution:
    def findAnagrams(self, s: str, p: str) -> List[int]:
        p_freq, w_freq = [0] * 26, [0] * 26
        res = []
        for ch in p:
        ...
        for right, ch in enumerate(s):
            ...
            if right - left + 1 == len(p): # 稳定大小的滑窗写法
                if p_freq == w_freq:
                    res.append(left)
                w_freq[ord(s[left]) - ord('a')] -= 1
                left += 1
        return res
```
...
一种写起来偷懒，但是复杂度太高的方法如下。

时间复杂度：$O(n \times m)$ ，空间复杂度：$O(1)$ ，其中 $n$ 和 $m$ 分别是 `s` 和 `p` 的长度。
...
class Solution:
    def findAnagrams(self, s: str, p: str) -> List[int]:
        ...
        for i in range(len(s) - len(p) + 1):
            if not (Counter(s[i:i + len(p)]) - target):
                res.append(i)
        return res
```
...
以某个位置（奇数/偶数）为中心的扩展计数子函数是很直接的。

我们只需要对所有可以为中心的点执行该函数就可以了。
...
其实这道题还有一个叫做Manacher的算法，时间 $O(n)$ ，空间 $O(n)$ ，但是难写，省略。
...
<div class="algoviz" data-module="lc560-和为-k-的子数组" data-title="560 和为 K 的子数组 · 步骤可视化"></div>
```
class Solution:
    def subarraySum(self, nums: List[int], k: int) -> int:
        n, res = len(nums), 0
...
<div class="algoviz" data-module="lc560-和为-k-的子数组-v2" data-title="560 和为 K 的子数组 · 步骤可视化"></div>
```
class Solution:
    def subarraySum(self, nums: List[int], k: int) -> int:
...
<div class="algoviz" data-module="lc76-最小覆盖子串" data-title="76 最小覆盖子串 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc53-最大子数组和" data-title="53 最大子数组和 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc53-最大子数组和-v2" data-title="53 最大子数组和 · 步骤可视化"></div>
```
class Solution:
    def maxSubArray(self, nums: List[int]) -> int:
        n = len(nums)
...
<div class="algoviz" data-module="lc189-轮转数组" data-title="189 轮转数组 · 步骤可视化"></div>
```
class Solution:
...
时间复杂度：$O(n)$  ，空间复杂度：$O(1)$ （如果我们计算复杂度时不考虑输出数组的话）

<div class="algoviz" data-module="lc189-轮转数组-v2" data-title="189 轮转数组 · 步骤可视化"></div>
```python
class Solution:
...
转化为求某个元素左/右边的乘积的问题，这当然是“前缀”的维护。

记得边界处理：最左边的元素没有左边的元素、最右边的元素没有右边的元素。
...
<div class="algoviz" data-module="lc41-缺失的第一个正数" data-title="41 缺失的第一个正数 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc41-缺失的第一个正数-v2" data-title="41 缺失的第一个正数 · 步骤可视化"></div>
```
class Solution:
    def firstMissingPositive(self, nums: List[int]) -> int:
        n = len(nums)
...
<div class="algoviz" data-module="lc73-矩阵置零" data-title="73 矩阵置零 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc73-矩阵置零-v2" data-title="73 矩阵置零 · 步骤可视化"></div>
```
class Solution:
    def setZeroes(self, matrix: List[List[int]]) -> None:
        """
...
<div class="algoviz" data-module="lc54-螺旋矩阵" data-title="54 螺旋矩阵 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc48-旋转图像" data-title="48 旋转图像 · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc240-搜索二维矩阵-ii" data-title="240 搜索二维矩阵 II · 步骤可视化"></div>
```
class Solution:
...
<div class="algoviz" data-module="lc240-搜索二维矩阵-ii-v2" data-title="240 搜索二维矩阵 II · 步骤可视化"></div>
```
class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
...
<div class="algoviz" data-module="lc160-相交链表" data-title="160 相交链表 · 步骤可视化"></div>
```
...
<div class="algoviz" data-module="lc206-反转链表" data-title="206 反转链表 · 步骤可视化"></div>
```
# Definition for singly-linked list.
# class ListNode:
...
<div class="algoviz" data-module="lc234-回文链表" data-title="234 回文链表 · 步骤可视化"></div>
```
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, val=0, next=None):
...
<div class="algoviz" data-module="lc141-环形链表" data-title="141 环形链表 · 步骤可视化"></div>
```
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, x):
...
<div class="algoviz" data-module="lc141-环形链表-v2" data-title="141 环形链表 · 步骤可视化"></div>
```
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, x):
...
<div class="algoviz" data-module="lc142-环形链表-ii" data-title="142 环形链表 II · 步骤可视化"></div>
```
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, x):
...
<div class="algoviz" data-module="lc2-两数相加" data-title="2 两数相加 · 步骤可视化"></div>
```
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, val=0, next=None):
...
<div class="algoviz" data-module="lc24-两两交换链表中的节点" data-title="24 两两交换链表中的节点 · 步骤可视化"></div>
```
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, val=0, next=None):
...
<div class="algoviz" data-module="lc138-随机链表的复制" data-title="138 随机链表的复制 · 步骤可视化"></div>
```
"""
...
<div class="algoviz" data-module="lc94-二叉树的中序遍历" data-title="94 二叉树的中序遍历 · 步骤可视化"></div>
```
...
时间复杂度：$O(n)$ ，空间复杂度：$O(n)$ （栈空间大小）

<div class="algoviz" data-module="lc94-二叉树的中序遍历-v2" data-title="94 二叉树的中序遍历 · 步骤可视化"></div>
```python
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
~~历史上有一个传闻，说某知名程序员因为面试谷歌时不会翻转二叉树被挂了：[Homebrew by Max Howell on X](https://x.com/mxcl/status/608682016205344768)~~

时间复杂度：$O(n)$ ，空间复杂度：$O(n)$ （上界是最坏情况，递归栈=深度=$n$） 

<div class="algoviz" data-module="lc94-二叉树的中序遍历-v3" data-title="94 二叉树的中序遍历 · 步骤可视化"></div>
```python
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
<div class="algoviz" data-module="lc101-对称二叉树" data-title="101 对称二叉树 · 步骤可视化"></div>
```
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
<div class="algoviz" data-module="lc543-二叉树的直径" data-title="543 二叉树的直径 · 步骤可视化"></div>
```
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
> 中序遍历的迭代写法是：
>
> 循环开头拿到**以当前节点为根的最左节点**，对它做操作，然后移动到下一个节点（如果不为空，根一定是它的右节点）。

每次拿到一个节点时，所判断的操作是：这个节点的值严格大于中序序列上一个节点的值。

注意循环条件：还有节点没判断。

时间复杂度：$O(n)$ ，空间复杂度：$O(n)$ ，因为栈开了这么大。

```python
class Solution:
    def isValidBST(self, root: Optional[TreeNode]) -> bool:
        prev = float('-inf') # 相当于我们在中序遍历序列
        stk = []
        node = root
        while stk or node:
            while node:
...
绝对不能只判断三个相邻节点值，而是判断值和**上下界**的关系。

节点值起到分隔出上下界的作用。

时间复杂度：$O(n)$ ，空间复杂度：$O(n)$ （递归栈深度取决于树深度，最坏情况树深度为节点数 $n$）
...
<div class="algoviz" data-module="lc199-二叉树的右视图" data-title="199 二叉树的右视图 · 步骤可视化"></div>
```
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
<div class="algoviz" data-module="lc437-路径总和-iii" data-title="437 路径总和 III · 步骤可视化"></div>
```
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
<div class="algoviz" data-module="lc437-路径总和-iii-v2" data-title="437 路径总和 III · 步骤可视化"></div>
```
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
<div class="algoviz" data-module="lc236-二叉树的最近公共祖先" data-title="236 二叉树的最近公共祖先 · 步骤可视化"></div>
```
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, x):
...
<div class="algoviz" data-module="lc124-二叉树中的最大路径和" data-title="124 二叉树中的最大路径和 · 步骤可视化"></div>
```
# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
...
要利用字符串唯一确定一棵二叉树，我们可能会想到数据结构中学习的利用x序+中序两种遍历方法。

但是如果直接写，很容易碰到问题：对于 `123` ，假如它是两个节点，应该解释成 `12` 和 `3` 还是 `1` 和 `23` ？

因此需要分隔符。

这里介绍另外一种更简便的方法：使用**前序+空节点标记+分隔符**，同样能唯一确定一棵二叉树。写法特别干净。

另外这道题的“运行”有bug，会显示 `name 'Or' is not defined` ，但是不用担心，提交是正常的。

时间复杂度：$O(n)$ ，空间复杂度：$O(n)$ （最坏情况深度为 $n$）
    ...
    def deserialize(self, data):
        ...
        def dfs():
            if vals[self.idx] == '#':
                self.idx += 1
                return None
            root = TreeNode(int(vals[self.idx])) # 根左右
            self.idx += 1
            root.left = dfs()
            root.right = dfs()
            return root
