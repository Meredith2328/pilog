---
title: 数据结构与算法 · 03 · 基础数据结构与算法模板整理
date: 2026-01-18 20:38:09
tags:
- 数据结构与算法
published: true
hideInList: false
...
（可参考：[数据结构（C++模板实现）](https://gitee.com/cyberdash/data-structures-cpp)）
...
![](posts/migrated/post-images/20260119095533.png)
```cpp
#include <iostream>
#include <cstdio>
#include <string>
...
![](posts/migrated/post-images/20260119103419.png)
```cpp
// 坐标系, 以及对格子的遍历.
...
![](posts/migrated/post-images/20260119111922.png)
```cpp
...
![](posts/migrated/post-images/20260119152217.png)
```cpp
// 1. 先把输入的整数逆序.
...
![](posts/migrated/post-images/20260119154237.png)
乘积位数不会超过两数的位数之和。
...
![](posts/migrated/post-images/20260119161520.png)
接下来给出封装好的大整数类，在 `Bigint` 结构体内重载了运算符 `operator[]` ，在结构体外`operator+` 和 `operator*` ，把上述两道题统一起来。
...
![](posts/migrated/post-images/20260119192004.png)
![](posts/migrated/post-images/20260119192103.png)
笔记：sort函数和unique函数（`#include <algorithm>`）
`sort(a, a+n, cmp)` 函数将数组 `a` 从 `a[0]` 到 `a[n - 1]` **原地排序**，默认升序。
`cmp` 可以直接定义一个bool的元素比较函数即可。
`unique(a, a+n)` 函数将将数组 `a` 从 `a[0]` 到 `a[n - 1]` **原地去重**，并且返回 `&a[n]` ，可以用返回值减去 `a` 得到**去重后的元素个数**。
### 举例

![](posts/migrated/post-images/20260119171116.png)
计数排序。
...
![](posts/migrated/post-images/20260119171804.png)
选择排序、冒泡排序、插入排序。
```cpp
// 选择排序:
...
（重点：快速排序的剪枝板子）
![](posts/migrated/post-images/20260119180906.png)
```cpp
// 快速排序板子.
// 划分 + 自顶向下分治.
...
![](posts/migrated/post-images/20260119180914.png)
备注：注意这里`MAXN = 5e6`，已经很大了，读入数字请用 `scanf` 而非 `cin` ，要不然会卡常（用例5搞出1.2s左右）。
...
![](posts/migrated/post-images/20260119191423.png)
（解一）范围小，所以用计数排序。
```cpp
#include <iostream>
#include <cstdio>
...
![](posts/migrated/post-images/20260119192747.png)
自定义比较函数。
```cpp
#include <iostream>
#include <cstdio>
...
![](posts/migrated/post-images/20260119194158.png)

![](posts/migrated/post-images/20260119194228.png)
原来已经统计完毕，直接输入的就是票数。
...
![](posts/migrated/post-images/20260127233859.png)
![](posts/migrated/post-images/20260127233909.png)
![](posts/migrated/post-images/20260127233915.png)
![](posts/migrated/post-images/20260127233921.png)
![](posts/migrated/post-images/20260127233927.png)
![](posts/migrated/post-images/20260127233938.png)
![](posts/migrated/post-images/20260127233945.png)
![](posts/migrated/post-images/20260127233954.png)
![](posts/migrated/post-images/20260127234007.png)
![](posts/migrated/post-images/20260127234012.png)
![](posts/migrated/post-images/20260127234022.png)
![](posts/migrated/post-images/20260127234036.png)
![](posts/migrated/post-images/20260127234043.png)
![](posts/migrated/post-images/20260127234059.png)
![](posts/migrated/post-images/20260127234105.png)
![](posts/migrated/post-images/20260127234111.png)

## 递推与递归专题

1. 递推和递归，要将原问题分解为子问题。递推往往考虑“最后一次”。
2. **递推**大概率结果很大，**要用 `long long` 甚至高精度**。

![](posts/migrated/post-images/20260119215619.png)
```
// 递推和初值
...
![](posts/migrated/post-images/20260119220826.png)
...
![](posts/migrated/post-images/20260119224913.png)
分析：
...
![](posts/migrated/post-images/20260119232743.png)
...
![](posts/migrated/post-images/20260120110235.png)

思路容易乱，做一个整理：从没有子问题、到有子问题。
1. 放松条件：不考虑递归，只有前面两个条件，则为 $n/2 + 1$ 。例如 $n = 6$ 时，其本身计数为1，在它后面可以添加 $n=3,2,1$ 三种情况，故计数为 $3+1=4$ 。
2. 添加条件：考虑递归。例如 $n = 6$ 时，之前是在 $n = 6$ 基础上添加了 $n=3,2,1$ 三个计数；现在把 $n=3,2,1$ 不看成每个问题计数为1（`result += 1`），而是**每个问题为需要调用函数计算的子问题**（`result += generate(i)`）。
3. 在写好原始递归后，将其升级为备忘录。
（ps. 下述这个树的每个节点代表一次函数调用）
![](posts/migrated/post-images/20260120112909.png)
...
![](posts/migrated/post-images/20260120112745.png)
写递归时，一定记得写备忘录。
```cpp
#include <iostream>
#include <cstdio>
...
![](posts/migrated/post-images/20260120115108.png)
由简到繁思考。
...
![](posts/migrated/post-images/20260120130210.png)
...
![](posts/migrated/post-images/20260120151617.png)
对于等待问题，画图出来会特别清晰。
![](posts/migrated/post-images/20260120151908.png)
...
![](posts/migrated/post-images/20260120154139.png)
...
![](posts/migrated/post-images/20260120155404.png)
![](posts/migrated/post-images/20260120175000.png)
这里讲解哈夫曼树，即每次选取最小的两个节点，合并、放回，如是循环，直到全部完成合并。

![](posts/migrated/post-images/20260120175211.png)
语法：优先队列
```cpp
...
![](posts/migrated/post-images/20260120181656.png)
...
![](posts/migrated/post-images/20260120181953.png)
```cpp
#include <iostream>
#include <cstdio>
using namespace std;
...
![](posts/migrated/post-images/20260120184539.png)
...
![](posts/migrated/post-images/20260120212003.png)
...
![](posts/migrated/post-images/20260120214210.png)
...
![](posts/migrated/post-images/20260120214536.png)
...
![](posts/migrated/post-images/20260120214549.png)
...
![](posts/migrated/post-images/20260121131056.png)

![](posts/migrated/post-images/20260121130720.png)
...
3. 坐标映射很像PyTorch的Tensor。函数参数使用的坐标是1~16，为了把它映射到行号和列号，需要先映射到0~15，然后使用整除`/`和取模`%`（本质是移位和位与）。而块号可以由行号和列号映射得到。当然也可以直接用原始坐标求得，见下。

这里提供一个位运算直接求得块号的过程，仅限1~16。
<div class="algoviz" data-module="lc3-坐标映射很像pytorch的tensor-函数参数使用的坐标是1-16-为了把它映射到行号和列号-需要先映射到0-15-然后使用整除-和取模-本质是移位和位与-而块号可以由行号和列号映射得到-当然也可以直接用原始坐标求得-见下" data-title="3 坐标映射很像PyTorch的Tensor。函数参数使用的坐标是1~16，为了把它映射到行号和列号，需要先映射到0~15，然后使用整除`/`和取模`%`（本质是移位和位与）。而块号可以由行号和列号映射得到。当然也可以直接用原始坐标求得，见下。 · 步骤可视化"></div>
```python
def map_number(n):
...
![](posts/migrated/post-images/20260121140234.png)
按行搜索，因为每一行只能放一个。
![](posts/migrated/post-images/20260121174824.png)
...
![](posts/migrated/post-images/20260121142934.png)
...
![](posts/migrated/post-images/20260121181514.png)
...
![](posts/migrated/post-images/20260121193716.png)
...
![](posts/migrated/post-images/20260121195806.png)
相当于知道每个格子被烧焦的时间。
剪枝条件：如果**目标格子的最早到达时间比被烧焦时间晚**，则不能扩展。
...
memset的头文件为cstring。
![](posts/migrated/post-images/20260120181150.png)

字符串的头文件为string。
![](posts/migrated/post-images/20260118204421.png)
7. **find的返回值备注**：`s.find(str [,pos])`在没找到时会返回`string::npos`, 需要强转为`int`才是-1, 否则直接输出得到 `size_t` 的最大值，可能是一个大数.

![](posts/migrated/post-images/20260118213050.png)
### 举例

以下三道题目利用字符串的七大常用方法，实现了字符计数、字符串处理和**单词统计**（空格法查找单词，给源文档和待查字符串前后都添加空格）。

![](posts/migrated/post-images/20260118204551.png)
```cpp
#include <iostream>
#include <string>
using namespace std;
...
![](posts/migrated/post-images/20260118204926.png)
```cpp
// 分析:
// 1. 分派指令.
...
![](posts/migrated/post-images/20260118211307.png)
```cpp
// 1. 读入但不区分大小写.
...
![](posts/migrated/post-images/20260118215605.png)
```cpp
#include <iostream>
#include <cstdio>
#include <string>
...
![](posts/migrated/post-images/20260118221515.png)
```cpp
...
![](posts/migrated/post-images/20260118222916.png)
...
![](posts/migrated/post-images/20260121222003.png)
![](posts/migrated/post-images/20260121222020.png)
stack在追求极致运行速度时，需要打开-O2优化。否则需要自己手写栈。
![](posts/migrated/post-images/20260121222214.png)
![](posts/migrated/post-images/20260121225021.png)
![](posts/migrated/post-images/20260122160844.png)
### 举例

![](posts/migrated/post-images/20260121220346.png)
...
![](posts/migrated/post-images/20260121220725.png)
...
![](posts/migrated/post-images/20260121222300.png)

![](posts/migrated/post-images/20260121222317.png)
...
![](posts/migrated/post-images/20260121223915.png)
...
![](posts/migrated/post-images/20260121225031.png)

![](posts/migrated/post-images/20260121225040.png)
```cpp
#include <iostream>
#include <cstdio>
#include <queue>
...
![](posts/migrated/post-images/20260122144025.png)
```cpp
...
![](posts/migrated/post-images/20260122123625.png)
前后插、维护index数组都很平常，
...
![](posts/migrated/post-images/20260122161006.png)
`#include <list>`
空初始化、利用数组初始化，
...
![](posts/migrated/post-images/20260122163942.png)
```cpp
int ans = 0;
void play(int n) {
...
![](posts/migrated/post-images/20260122164847.png)
```cpp
#include <iostream>
#include <stack>
using namespace std;
...
![](posts/migrated/post-images/20260122182400.png)
![](posts/migrated/post-images/20260122182845.png)
...
![](posts/migrated/post-images/20260122205202.png)
这是使用之前的孩子-双亲表示法的开法。
```cpp
#include <iostream>
#include <cstdio>
...
![](posts/migrated/post-images/20260122211916.png)

![](posts/migrated/post-images/20260122211940.png)
```cpp
// eg. 前序: [1] 2 3 4 5  6  7
//     中序:  4  3 5 2 6 [1] 7
// 递归处理字符串序列即可
...
![](posts/migrated/post-images/20260122213623.png)
...
![](posts/migrated/post-images/20260122215047.png)
每个节点存left, right, father和value。
![](posts/migrated/post-images/20260123171508.png)
...
![](posts/migrated/post-images/20260122215225.png)
...
![](posts/migrated/post-images/20260124205529.png)
![](posts/migrated/post-images/20260124211151.png)
### 举例

并查集仅仅只是一个 `fa` 数组和两个函数。
其中最关键的是：**路径压缩**。（查询降低至 $O(\log n)$ ）
即`find`除了找到根，还要把一路上每一个节点的`fa`都设成根。


![](posts/migrated/post-images/20260123205939.png)
路径压缩的两种写法：
```cpp
int find(int x) {
    if (x == fa[x]) return x;
...
![](posts/migrated/post-images/20260123212113.png)
...
![](posts/migrated/post-images/20260123213719.png)
...
![](posts/migrated/post-images/20260123213448.png)
这里的知识重点是怎么把字符串转成哈希。
对于字符串哈希，在模数mod（23333）的基础上还引入了基数base（261）。

![](posts/migrated/post-images/20260123215548.png)
```cpp
#include <iostream>
#include <string>
#include <vector>
...
![](posts/migrated/post-images/20260123220911.png)
只用到前两个字母。
...
![](posts/migrated/post-images/20260124205321.png)
插入，存在，前驱，后继，删除。
...
![](posts/migrated/post-images/20260124211135.png)
```cpp
#include <iostream>
#include <cstdio>
#include <map>
...
![](posts/migrated/post-images/20260124212532.png)
```cpp
#include <iostream>
#include <cstdio>
...
![](posts/migrated/post-images/20260126202028.png)
![](posts/migrated/post-images/20260126202017.png)
...
![](posts/migrated/post-images/20260126202709.png)
```cpp
#include <iostream>
#include <vector>
using namespace std;
...
![](posts/migrated/post-images/20260126203255.png)
dfs：先设置vis，后调用dfs实现相应逻辑。
bfs：注意bfs中的vis是指是否入队，所以**vis要在入队前标记**。
而打印是通过取出队列元素实现bfs顺序，所以**对节点的操作要写在循环体以内**。
...
![](posts/migrated/post-images/20260127132307.png)
![](posts/migrated/post-images/20260127132408.png)
要处理“遍历顺序”。
...
![](posts/migrated/post-images/20260127133917.png)
完成每个任务的最短时间，
...
![](posts/migrated/post-images/20260127134741.png)

![](posts/migrated/post-images/20260127211832.png)
...
![](posts/migrated/post-images/20260127213257.png)
先转到10进制，然后从十进制转到该进制。
...
![](posts/migrated/post-images/20260127215600.png)
![](posts/migrated/post-images/20260127220353.png)
...
![](posts/migrated/post-images/20260127215619.png)
...
![](posts/migrated/post-images/20260127215630.png)
在杨辉三角打表的同时，对 $k$ 取模。
![](posts/migrated/post-images/20260127221517.png)
![](posts/migrated/post-images/20260127221550.png)
```cpp
#include <iostream>
#include <algorithm>
using namespace std;
...
![](posts/migrated/post-images/20260127232119.png)
![](posts/migrated/post-images/20260127233229.png)
![](posts/migrated/post-images/20260127233247.png)
### 举例

添加了新的工具：枚举**约数**。
方法是从 $1$ 枚举到 $\sqrt{n}$ 。
![](posts/migrated/post-images/20260127222207.png)
```cpp
#include <iostream>
using namespace std;
...
```
![](posts/migrated/post-images/20260127222719.png)
例如原长度为12，则找约数是 $O(\sqrt{n})$ 的，枚举检验长度为1, 2, 3, 4, 6, 12的字符串是 $O(Pn)$ 的，其中 $P$ 是约数个数6。
![](posts/migrated/post-images/20260127222847.png)

![](posts/migrated/post-images/20260127222858.png)
如果枚举每对数， $O(n^2)$ 。
...
![](posts/migrated/post-images/20260127224141.png)
...
![](posts/migrated/post-images/20260127225805.png)
![](posts/migrated/post-images/20260127230027.png)
```cpp
#include <cstdio>
#include <cstring>
#include <algorithm>
...
![](posts/migrated/post-images/20260127232237.png)
![](posts/migrated/post-images/20260127232255.png)
![](posts/migrated/post-images/20260127232305.png)

![](posts/migrated/post-images/20260127232406.png)

![](posts/migrated/post-images/20260127232453.png)
![](posts/migrated/post-images/20260127232752.png)
![](posts/migrated/post-images/20260127232758.png)
```cpp
#include <cstdio>
using namespace std;
int gcd(int x, int y) {
...
![](posts/migrated/post-images/20260127232809.png)
```cpp
#include <cstdio>
using namespace std;
int gcd(int x, int y) {
...
![](posts/migrated/post-images/20260127233324.png)
```cpp
#include <bits/stdc++.h>
using namespace std;
#define MAXN 10010
...
![](posts/migrated/post-images/20260127225831.png)
初步学习完结撒花！

上一篇：[[algorithm-2|02 算法篇]] · 下一篇：[[algorithm-4|04 HOT 100（前 50）]]
