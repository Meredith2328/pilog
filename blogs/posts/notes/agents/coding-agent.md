...
<div class="archviz">
  <div class="av-head"><span class="av-tag">架构图</span><span class="av-title">主循环分层状态机：pi-agent-core 四层 + 层间真实连接方式（蓝条 = 向下调用，绿条 = 向上事件）</span></div>
  <div class="av-body">
    <div class="av2">
      <div class="av2-main">
        <div class="av2-lane">
          <div class="av2-lane-head"><span class="av2-lane-tag">L1 Agent 会话层</span><span class="av2-lane-sub">agent-session.js + agent.js：一次用户提交到 agent_settled</span></div>
          <div class="av2-node" data-av2-layer="agent">
            ...
            <details class="av-fold"><summary>代码<a class="av-loc" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L346-L385">agent.ts:346-385</a></summary><div class="av-fold-body"><pre>async prompt(input, images) {
    if (this.activeRun) {
...
followUp(message) { this.followUpQueue.enqueue(message); }</pre><code>skipInitialSteeringPoll</code>（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L442">agent.ts:442</a> / <a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L470-L476">agent.ts:470-476</a>）防同一条 steering 被注入两遍。</div></details>
          </div>
          <div class="av2-conn">
            <div class="av2-conn-arrows">↓<br>↑</div>
            <div class="av2-conn-body"><b>调用</b>：runPromptMessages → runWithLifecycle(signal) → <b>await</b> runAgentLoop(...)（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L405-L419">agent.ts:405-419</a>）｜<b>返回</b>：newMessages[]，异常经 handleRunFailure（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L507">agent.ts:507-507</a>）转成合成的 agent_end 事件</div>
          </div>
          <div class="av2-node" data-av2-layer="agent">
            <div class="av-name">createLoopConfig：把 Agent 的队列变成循环的钩子</div>
            <div class="av-sub">steering/followUp 队列的 drain 函数在这里注入循环（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L441-L480">agent.ts:441-480</a>）</div>
          </div>
        </div>

        <div class="av2-conn up">
          <div class="av2-conn-arrows">↑</div>
          <div class="av2-conn-body"><b>事件流（唯一向上的通道）</b>：processEvents 把 agent_start / turn_start / message_* / tool_execution_* / turn_end / agent_end 逐个 emit 给 AgentSession 的订阅者（外层再转发给扩展和 TUI）。事件与调用同一条 emit 链，顺序即发生顺序。</div>
        </div>

        <div class="av2-lane">
          <div class="av2-lane-head"><span class="av2-lane-tag">L2 Turn 循环层</span><span class="av2-lane-sub"><a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L163-L278">agent-loop.ts:163-278</a> runLoop：双层 while，一个 turn = 一次 LLM 调用 + 一个工具批次</span></div>
          ...
          <div class="av2-node" data-av2-layer="turn">
            ...
            <details class="av-fold"><summary>内层循环原文（省略号处见源码）<a class="av-loc" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L174-L261">agent-loop.ts:174-261</a></summary><div class="av-fold-body"><pre>while (hasMoreToolCalls || pendingMessages.length > 0) {
    ...
    pendingMessages =
        (await config.getSteeringMessages?.()) || [];         // :160 捞steering
}</pre>三个 agent_end 出口：:110（error/aborted）、:157（钩子叫停）、:172（follow-up 耗尽）。</div></details>
          </div>
          <div class="av2-conn">
            <div class="av2-conn-arrows">↓<br>↑</div>
          ...
          <div class="av2-node" data-av2-layer="msg">
            ...
            <details class="av-fold"><summary>翻译边界的四步<a class="av-loc" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L281-L379">agent-loop.ts:281-379</a></summary><div class="av-fold-body"><pre>let messages = context.messages;
...
const response = await streamFunction(
    config.model, llmContext, { ...config, apiKey: resolvedApiKey, signal });</pre>文件头注释点明设计意图："Agent loop that works with AgentMessage throughout. Transforms to Message[] only at the LLM call boundary."——会话格式与协议格式只在这一点交汇。</div></details>
          </div>
          <div class="av2-conn">
            <div class="av2-conn-arrows">↓<br>↑</div>
          ...
          <div class="av2-lane">
            <div class="av2-lane-head"><span class="av2-lane-tag">L4 Tool 执行层</span><span class="av2-lane-sub"><a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L381-L792">agent-loop.ts:381-792</a>：一个工具批次的完整流水线</span></div>
            <div class="av2-node" data-av2-layer="tool">
              ...
              <details class="av-fold"><summary>分岔原文<a class="av-loc" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L411-L426">agent-loop.ts:411-426</a></summary><div class="av-fold-body"><pre>const hasSequentialToolCall = toolCalls.some((tc) =>
    currentContext.tools?.find((t) => t.name === tc.name)
        ?.executionMode === "sequential");
if (config.toolExecution === "sequential" || hasSequentialToolCall) {
    return executeToolCallsSequential(...);
}
return executeToolCallsParallel(...);</pre></div></details>
            </div>
            <div class="av2-conn">
              <div class="av2-conn-arrows">↓</div>
            ...
            <div class="av2-node" data-av2-layer="tool">
              ...
              <details class="av-fold"><summary>并行执行原文<a class="av-loc" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L544-L558">agent-loop.ts:544-558</a></summary><div class="av-fold-body"><pre>const orderedFinalizedCalls = await Promise.all(
...
}</pre>顺序稳定性：map 的数组下标 = toolCall 原始顺序，Promise.all 不改位置。</div></details>
            </div>
            <div class="av2-node" data-av2-layer="tool">
              <div class="av-name">单个工具内部：execute + 流式上报 + afterToolCall 改写<span class="av-loc">:449-514</span></div>
              <div class="av-sub">tool.execute 第四参是 onUpdate 回调——bash 的 100ms 节流输出从这里流出去（:453-463）；跑完过 afterToolCall 钩子，可改写 content/details/usage/isError/terminate（:480-513）</div>
            </div>
          </div>
        </div>
      </div>
...
<div class="archviz">
  <div class="av-head"><span class="av-tag">架构图</span><span class="av-title">按目的演进：runLoop 的全部零件（①→④ 每步只加新件，虚线框 = 本阶段新增）</span></div>
  <div class="av-body">
    <div class="av3-stages">
      <button type="button" class="av3-tab" data-av3-stage="qa"><span class="av-ref">①</span> 纯消息问答</button>
      <button type="button" class="av3-tab" data-av3-stage="tools"><span class="av-ref">②</span> ＋工具调用</button>
      <button type="button" class="av3-tab" data-av3-stage="robust"><span class="av-ref">③</span> ＋错误处理与截断</button>
      <button type="button" class="av3-tab" data-av3-stage="full"><span class="av-ref">④</span> ＋steering · 换模型 · 续跑</button>
    </div>

    <div class="av3-pane" data-av3-stage="qa">
      <div class="av3-note">只做问答：一个 turn 就够——上下文变换、调 LLM、收回消息、结束。连 while 都不转（<code>hasMoreToolCalls=false</code> 且无 pending，:88 条件为假）。但即便这个最小形态，源码里也有两件容易被忽略的必需品：AgentMessage→Message 的翻译边界，和每次调用现解析 API key。</div>
      <div class="av-flow">
        <div class="av-stage av-node k-core">
          <div class="av-name">transformContext（可选）<span class="av-loc">:181-183</span></div>
          <div class="av-sub">调用 LLM 前最后机会变换 AgentMessage[]（压缩历史、注入提示都可以挂这）</div>
        </div>
        <div class="av-down"><span class="av-alabel">convertToLlm :185</span></div>
        <div class="av-stage av-node k-core">
          <div class="av-name">convertToLlm → llmContext<span class="av-loc">:185-191</span></div>
          <div class="av-sub">AgentMessage[] 翻译成协议 Message[]——会话格式与协议格式唯一的交汇点</div>
        </div>
        <div class="av-down is-accent"><span class="av-alabel">getApiKey :193</span></div>
        <div class="av-stage av-node">
          <div class="av-name">streamFunction(model, ctx, opts)<span class="av-loc">:194-198</span></div>
          <div class="av-sub">每次调用都 await getApiKey 现解析——OAuth token 过期靠这个无缝续命，不必重启会话</div>
          <details class="av-fold"><summary>这四步的原文</summary><div class="av-fold-body"><pre>let messages = context.messages;
...
const response = await streamFunction(config.model, llmContext,
    { ...config, apiKey: resolvedApiKey, signal });</pre></div></details>
        </div>
        <div class="av-down"><span class="av-alabel">message_end :240</span></div>
        <div class="av-stage av-node k-end">
          <div class="av-name">stopReason === "stop" → 结束</div>
          <div class="av-sub">turn_end → follow-up 检查（空）→ agent_end（:172）</div>
        </div>
      </div>
    </div>

    <div class="av3-pane" data-av3-stage="tools">
      <div class="av3-note">加工具调用：while 开始真正转。新增三件——toolCall 过滤、工具批次执行（默认并行 + 顺序回填）、terminate 投票。前两件是回环，第三件是"某个工具可以叫停整个 agent"的民主机制。</div>
      <div class="av-flow">
        <div class="av-stage av-node k-core">
          <div class="av-name">streamAssistantResponse 返回</div>
          <div class="av-sub">stopReason 现在可能是 toolUse，content 里有 1..N 个 toolCall 块</div>
        </div>
        <div class="av-down is-accent"><span class="av-alabel">filter :114</span></div>
        <div class="av-stage av-node av3-delta">
          <div class="av-name">executeToolCalls：策略分岔<span class="av-loc">:287-294</span></div>
          <div class="av-sub">默认并行（Promise.all + thunk），sequential 工具在场才串行</div>
          <details class="av-fold"><summary>新增的核心几行<span class="av-loc">:114-129</span></summary><div class="av-fold-body"><pre>const toolCalls = message.content
...
if (toolCalls.length > 0) {
    ...
    for (const result of toolResults) {
        ...
        <div class="av-down"><span class="av-alabel">下一圈</span></div>
        <div class="av-stage av-node k-data">
          <div class="av-name">带工具结果的上下文再进 LLM</div>
          <div class="av-sub">tool_use/tool_result 配对顺序由 :365-371 的 map 下标保证</div>
        </div>
        <div class="av-rail"><span class="av-rlabel">TOOL RESULT 回环</span></div>
      </div>
    </div>

    <div class="av3-pane" data-av3-stage="robust">
      <div class="av3-note">加错误处理与截断：全部长在固定分支上，循环骨架不动。三种 stopReason 特殊值 + 一个合成事件出口，覆盖"模型出错、用户中断、输出截断、循环本身崩了"四种失败模式。</div>
      <div class="av-layers">
        <div class="av-layer l-core">
          ...
          <div class="av-l-desc">turn_end（带空 toolResults）+ agent_end 立即返回。重试不在这层做——外层 agent-session 的 _prepareRetry（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/coding-agent/src/core/agent-session.ts">agent-session.ts:764 区段</a>）决定要不要 continue() 重来。</div>
        </div>
    ...
    return { messages, terminate: false };
}</pre>注释原文点破了动机："a truncated message can yield tool calls whose arguments parse and validate but are silently incomplete. None of them are safe to execute."</div></details>
        </div>
        <div class="av-l-arrow"><span class="av-alabel">循环抛异常</span></div>
        <div class="av-layer l-bus">
          <div class="av-l-head"><span class="av-l-name">handleRunFailure：合成 agent_end<a class="av-loc" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L482-L505">agent.ts:482-505</a></span><span class="av-badge">＋新增</span></div>
        ...
        <div class="av-layer l-front">
          <div class="av-l-head"><span class="av-l-name">abort：合作式检查点<span class="av-loc">:412/:427/:323/:350</span></span><span class="av-badge">＋新增</span></div>
          <div class="av-l-desc">signal 传进流式调用（打断 HTTP）和每个工具；beforeToolCall 返回后、串行批次每个工具之间都查一次。不是循环顶部的 if，是每个 await 点的礼貌询问。</div>
        </div>
      </div>
    </div>

    <div class="av3-pane" data-av3-stage="full">
      <div class="av3-note">加 steering、换模型、续跑：runLoop 的最后一批挂件。到这一步，agent-loop.js 的 548 行就全部出现在图上了——没有剩余的"边角代码"。</div>
      <div class="av-layers">
        <div class="av-layer l-bus">
          ...
          <details class="av-fold"><summary>steering 的完整路径</summary><div class="av-fold-body">入队：AgentSession._queueSteer（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/coding-agent/src/core/agent-session.ts">agent-session.ts:1016 区段</a>）→ agent.steer()（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L283">agent.ts:283</a>）→ steeringQueue。消费：createLoopConfig 的 getSteeringMessages（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L470-L476">agent.ts:470-476</a>）→ runLoop <a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L168">:168</a>/<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L258">:258</a> → <a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent-loop.ts#L182-L190">:182-190</a> 注入。UI 侧 _steeringMessages 数组跟踪未送达消息，message_start 确认送达后移除（agent-session.js）。continue() 在 assistant 结尾时优先 drain 队列（<a class="av-link" href="https://github.com/earendil-works/pi/blob/v0.84.0/packages/agent/src/agent.ts#L363-L383">agent.ts:363-383</a>）。</div></details>
        </div>
        <div class="av-l-arrow"><span class="av-alabel">turn_end 之后</span></div>
        <div class="av-layer l-core">
          <div class="av-l-head"><span class="av-l-name">prepareNextTurn：turn 间换模型<span class="av-loc">:138-150</span></span><span class="av-badge">＋新增</span></div>
          <div class="av-l-desc">每个 turn 结束调用，返回快照可替换 context、换 model、调 thinkingLevel（"off" 映射为 undefined）。会话中途 /model 切换不掉线的原因。</div>
          <details class="av-fold"><summary>换模型的原文<span class="av-loc">:138-150</span></summary><div class="av-fold-body"><pre>const nextTurnSnapshot =
...
if (nextTurnSnapshot) {
    ...
    config = {
        ...
        <div class="av-l-arrow"><span class="av-alabel">内层退出后</span></div>
        <div class="av-layer l-infra">
          <div class="av-l-head"><span class="av-l-name">外层 while + follow-up 续跑<span class="av-loc">:85, :163-170</span></span><span class="av-badge">＋新增</span></div>
          <div class="av-l-desc">agent 本要停时捞 follow-up 队列，有 → pendingMessages = followUpMessages; continue（:164-167）回内层。外层 while(true) 存在的全部理由。</div>
        </div>
        <div class="av-l-arrow"><span class="av-alabel">更外层</span></div>
        <div class="av-layer l-ext">
          <div class="av-l-head"><span class="av-l-name">AgentSession 的善后方向盘<span class="av-loc">agent-session.js:744-790</span></span></div>
          <div class="av-l-desc">_runAgentPrompt：while (await _handlePostAgentRun()) agent.continue()。_handlePostAgentRun 按优先级：可重试错误 → _prepareRetry（2s/4s/8s 退避）；_checkCompaction（16384 reserveTokens 阈值触发压缩）；hasQueuedMessages。这一层不在 agent-loop.js 里，但每个 runLoop 的使用者都要回答"然后呢"。</div>
          <details class="av-fold"><summary>外层方向盘原文<span class="av-loc">agent-session.js:744-790</span></summary><div class="av-fold-body"><pre>async _runAgentPrompt(messages) {
...
async _handlePostAgentRun() {
  ...
  <div class="av-foot">①翻译边界与 key 解析是问答就有的必需品；②回环 + terminate 投票；③四种失败模式各有专属分支；④全部挂件就位后 agent-loop.js 548 行完整覆盖。阶段④最外层跳出 agent-loop.js，指向使用者（AgentSession）的善后循环。</div>
</div>
